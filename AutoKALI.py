...
def InstallUtilities(package_manager: str, utilities_list_path: Path):
    if package_manager != "pacman":
        logging.error(f"Package manager '{package_manager}' not supported.")
        sys.exit(1)
...
def UninstallUtilities(package_manager: str, utilities_list_path: Path):
    if package_manager != "pacman":
        logging.error(f"Package manager '{package_manager}' not supported.")
        sys.exit(1)
...
def InstallKalitheme(package_manager: str):
    ...
    if packages_to_install:
        ...
        InstallUtilities(package_manager, KALITHEME_PACKAGES_TXT)
    else:
...
def UninstallKalitheme(package_manager: str):
    ...
    if packages_to_uninstall:
        ...
        UninstallUtilities(package_manager, KALITHEME_PACKAGES_TXT)
    else:
...
def main():
    ...
    p_install.add_argument("package_manager", choices=["pacman"], help="The package manager to use.")
    p_install.add_argument("utilities_list", type=Path, help="Path to the .txt file with the list of utilities.")
    p_install.set_defaults(func=lambda args: InstallUtilities(args.package_manager, args.utilities_list))

    p_uninstall = subparsers.add_parser("uninstall-utilities", help="Uninstall utilities from a list.")
    p_uninstall.add_argument("package_manager", choices=["pacman"], help="The package manager to use.")
    p_uninstall.add_argument("utilities_list", type=Path, help="Path to the .txt file with the list of utilities.")
    p_uninstall.set_defaults(func=lambda args: UninstallUtilities(args.package_manager, args.utilities_list))

    p_install_theme = subparsers.add_parser("install-kalitheme", help="Apply the Kali theme.")
    p_install_theme.add_argument("package_manager", choices=["pacman"], help="The package manager to use.")
    p_install_theme.set_defaults(func=lambda args: InstallKalitheme(args.package_manager))
    
    p_uninstall_theme = subparsers.add_parser("uninstall-kalitheme", help="Remove the Kali theme and restore backups.")
    p_uninstall_theme.add_argument("package_manager", choices=["pacman"], help="The package manager to use.")
    p_uninstall_theme.set_defaults(func=lambda args: UninstallKalitheme(args.package_manager))
